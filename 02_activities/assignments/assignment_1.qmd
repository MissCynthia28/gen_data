...
# Create SNP list file with 4 SNPs
...
geno_subset_A1 <- fread("./02_activities/data/geno.A1.additive.raw")
...
```{r,message=FALSE, warning=FALSE}
set.seed(1234)
fam_all_A1 <- fread("./02_activities/data/gwa.qc.A1.fam")
N=dim(fam_all_A1)[1];N
bim_all_A1 <- fread("./02_activities/data/gwa.qc.A1.bim")
M <- nrow(bim_all_A1);M

indx.SNP=which((bim_all_A1$V2)%in%c('rs1861','rs3813199','rs3128342','rs11804831'))
#print(indx.SNP)
geno_mat_A1 <- readPlinkToMatrixByIndex('./02_activities/data/gwa.qc.A1', 1:N, indx.SNP)
dim(geno_mat_A1)
head(geno_mat_A1)
```

```{r,message=FALSE, warning=FALSE}
geno_mat_A1[geno_mat_A1==-9]=NA
# Compute allele frequencies for each SNP (column), ignoring NAs
allele_freq <- apply(geno_mat_A1, 2, function(x) {
...
#rs1861 = 0.9430874; rs3813199 = 0.8456588; rs3128342 = 0.6948789; rs11804831 = 0.9460141

(ii) What are the minor allele frequencies (MAFs) for these four SNPs?


# Your code here...
rs1861 = 1- 0.9430874 = 0.0567126  
rs3813199 = 1 - 0.8456588 = 0.1543412
rs3128342 = 1 - 0.6948789 = 0.3051211
rs11804831 = 1 - 0.9460141 = 0.0539859
```
...
hwe_pvalues_A1 <- apply(geno_mat_A1, 2, hwe_pvalue)
...
```{r,message=FALSE, warning=FALSE}
#Pick SNP rs1861
table(geno_subset_A1$rs1861)
freq[freq$ID=='rs1861',]
```

```{r,message=FALSE, warning=FALSE}
# Fit linear regression model
model <- lm(PHENOTYPE ~ rs1861_C, data = geno_subset_A1)

# View model summary
summary(model)
table(geno_subset_A1$	rs1861_C)
```
#p-value < 2e-16
...
```         
# Your answer here... We performed linear regression to test the association between genotype (rs1861_C) and the continous phenotype. The regression coefficient for rs1861 is 0.97382 (p <2e-16), indicating that each additional copy of the T allele is associated with an average increase of 0.97382 units of the phenotype. This suggests a positive additive effect. The significant p-value suggests evidence of an association between this SNP and the phenotype in our sample.
```
...
```
```{r,message=FALSE, warning=FALSE}
plot(jitter(geno_subset_A1$rs1861_C), geno_subset_A1$PHENOTYPE,
     xlab = "Genotype (rs1861_C: 0,1,2 copies)",
     ylab = "Continuous phenotype",
     main = "Phenotype vs rs1861_C genotype",
     pch = 16)
...
```
```{r}
geno_subset_dom_A1 <- geno_subset_A1  
geno_subset_dom_A1[, 7:ncol(geno_subset_dom_A1)] <- as.data.frame(
  lapply(geno_subset_A1[, 7:ncol(geno_subset_A1)], function(x) ifelse(x == 2, 1, 0))
)
```
...
model <- lm(PHENOTYPE ~ rs1861_C, data = geno_subset_dom_A1)
summary(model)
```
...
```
```{r,message=FALSE, warning=FALSE}
plot(jitter(geno_subset_dom_A1$rs1861_C), geno_subset_dom_A1$PHENOTYPE,
     xlab = "Genotype",
     ylab = "Continuous phenotype",
     main = "Phenotype vs rs1861_C genotype",
     pch = 16)
...
```         
# Your answer here... The R square of 0.08924, which is closer to one, indicates that the additive model offers the best overall fit.
```
